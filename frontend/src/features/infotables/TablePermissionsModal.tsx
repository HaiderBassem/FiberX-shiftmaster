...
export const TablePermissionsModal = ({ isOpen, onClose }: TablePermissionsModalProps) => {
  ...
  const togglePermission = useMutation({
    mutationFn: async ({ id, canCreate }: { id: string; canCreate: boolean }) => {
      await api.put(`/employees/${id}/table-permission`, {
        can_create_tables: canCreate,
      });
    },
    onSuccess: () => {
      ...
      <div className="bg-card w-full max-w-lg rounded-xl shadow-lg border border-border flex flex-col max-h-[85vh]">
        <div className="flex items-center justify-between p-4 border-b border-border">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold">Manage Table Permissions</h2>
          </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        ...
                        checked={emp.can_create_tables}
                        onChange={(e) => togglePermission.mutate({ id: emp.id, canCreate: e.target.checked })}
