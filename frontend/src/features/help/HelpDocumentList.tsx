  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">مستندات المساعدة</h1>
          <p className="text-gray-500 text-sm mt-1">الوصول السريع للأدلة والإرشادات الخاصة بقسمك</p>
        </div>
      ...
      {documents.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-1">لا توجد مستندات</h3>
          <p className="text-gray-500 max-w-sm mx-auto">
            لم يتم إضافة أي مستندات مساعدة لقسمك حتى الآن، أو لم يتم منحك صلاحية الوصول إليها.
          </p>
        </div>
      ) : (
          ...
          {documents.map((doc) => (
            ...
            >
              ...
              <div className="text-xs text-gray-500 mt-4 flex items-center justify-between border-t border-gray-100 pt-4">
                <span>تاريخ الإضافة: {format(new Date(doc.created_at), 'yyyy-MM-dd')}</span>
              </div>
            </div>
          ))}
        </div>
