...
export const Sidebar = ({ onClose }: { onClose?: () => void }) => {
  ...
  return (
    <aside className="w-64 h-screen bg-sidebar text-sidebar-foreground flex flex-col border-r border-border/30">
      ...
      {myModules && myModules.length > 0 && (
        <div className="px-3 pb-3">
          <div className="p-2 bg-white/5 border border-white/5 rounded-xl space-y-1">
            ...
            {myModules.filter(link => !(['Live Map', 'Ticket System'].includes(link.title) || link.url.includes('maps.shift-master.org') || link.url.includes('ticket.shift-master.org'))).map(link => {
              const Icon = (() => {
      ...
      {/* ── Live Map Tab ── */}
      {myModules?.some(link => link.title === 'Live Map' || link.url.includes('maps.shift-master.org')) && (
      <div className="px-3 pb-3">
        <button
          id="live-map-tab"
          onClick={() => {
      ...
      {/* ── Ticket System Tab ── */}
      {myModules?.some(link => link.title === 'Ticket System' || link.url.includes('ticket.shift-master.org')) && (
      <div className="px-3 pb-3">
        <button
