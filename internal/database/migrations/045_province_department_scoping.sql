...
ALTER TABLE provinces ADD COLUMN IF NOT EXISTS department_id UUID REFERENCES departments(id) ON DELETE CASCADE;
ALTER TABLE provinces ADD COLUMN IF NOT EXISTS created_by UUID REFERENCES employees(id) ON DELETE SET NULL;

-- Assign a fallback department to existing provinces so we can apply NOT NULL.
-- On a fresh install this matches nothing: 043 only seeds provinces once a department
-- exists, so the table is empty here and SET NOT NULL applies cleanly either way.
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_provinces_dept_name ON provinces(department_id, name);
...
ALTER TABLE service_categories ADD COLUMN IF NOT EXISTS province_id UUID REFERENCES provinces(id) ON DELETE CASCADE;
