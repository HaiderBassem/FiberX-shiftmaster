...
// TaskRepository handles task schedules, assignments, and executions.
type TaskRepository interface {
	...
	CompleteExecution(ctx context.Context, id uuid.UUID, notes *string) error
}
...
func (r *taskRepo) GetMyWeeklyTasks(ctx context.Context, employeeID uuid.UUID, weekStart, weekEnd time.Time) ([]models.MyTaskRow, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&t.AssignmentID, &t.AssignedDate, &t.TaskTitle, &t.TaskDesc,
			...
			&t.ExecutionID, &t.Status, &t.StartedAt, &t.CompletedAt, &t.Notes); err != nil {
...
func (r *taskRepo) CompleteExecution(ctx context.Context, id uuid.UUID, notes *string) error {
	_, err := r.db.Exec(ctx,
		`UPDATE task_executions SET status='completed', completed_at=CURRENT_TIMESTAMP, notes=$1, updated_at=CURRENT_TIMESTAMP WHERE id=$2`,
		notes, id)
	return err
}
