...
func (r *departmentRepo) GetByID(ctx context.Context, id uuid.UUID) (*models.Department, error) {
	...
	err := r.db.QueryRow(ctx,
		`SELECT id, department_code, name, description, COALESCE(fiberx_enabled, false), max_leaves_per_day, active_modules, created_at, updated_at
		 FROM departments WHERE id = $1`, id,
	).Scan(&dept.ID, &dept.DepartmentCode, &dept.Name, &dept.Description,
		&dept.FiberxEnabled, &dept.MaxLeavesPerDay, &dept.ActiveModules, &dept.CreatedAt, &dept.UpdatedAt)
	if err != nil {
...
func (r *departmentRepo) GetAll(ctx context.Context) ([]models.Department, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&dept.ID, &dept.DepartmentCode, &dept.Name, &dept.Description,
			&dept.FiberxEnabled, &dept.MaxLeavesPerDay, &dept.ActiveModules, &dept.CreatedAt, &dept.UpdatedAt); err != nil {
...
func (r *departmentRepo) Create(ctx context.Context, dept *models.Department) error {
	err := r.db.QueryRow(ctx,
		...
		dept.DepartmentCode, dept.Name, dept.Description, dept.MaxLeavesPerDay, dept.ActiveModules,
...
func (r *departmentRepo) Update(ctx context.Context, dept *models.Department) error {
	_, err := r.db.Exec(ctx,
		`UPDATE departments SET name=$1, description=$2, max_leaves_per_day=$3, active_modules=$4, updated_at=CURRENT_TIMESTAMP
		 WHERE id=$5`,
		dept.Name, dept.Description, dept.MaxLeavesPerDay, dept.ActiveModules, dept.ID)
	if err != nil {
		return err
	}
...
func (r *departmentRepo) GetByManagerID(ctx context.Context, managerID uuid.UUID) ([]models.Department, error) {
	rows, err := r.db.Query(ctx,
		`SELECT d.id, d.department_code, d.name, d.description, COALESCE(d.fiberx_enabled, false), d.max_leaves_per_day, d.active_modules, d.created_at, d.updated_at
		 FROM departments d
